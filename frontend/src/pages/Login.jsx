...
const BgBlob = () => {
  ...
  return (
    <mesh ref={ref} scale={3} position={[0, 0, -5]}>
      <sphereGeometry args={[1, 32, 32]} />
      <MeshDistortMaterial color="#d4a373" distort={0.45} speed={2} roughness={0.6} metalness={0.2} opacity={0.6} transparent />
    </mesh>
  );
};
    ...
    >
      ...
      <div className="absolute inset-0 z-0 opacity-20">
        <Canvas camera={{ position: [0, 0, 5] }}>
          ...
          <pointLight position={[5, 5, 5]} intensity={1.2} color="#d4a373" />
          <BgBlob />
        </Canvas>
      </div>
      {/* Warm overlay */}
      <div className="absolute inset-0 z-1" style={{ background: "linear-gradient(135deg, rgba(254,250,224,0.5), rgba(250,237,205,0.7))" }} />

      {/* Orb accents */}
      <div className="absolute top-1/4 left-1/3 w-72 h-72 rounded-full blur-[90px] pointer-events-none z-1"
        style={{ background: "rgba(212,163,115,0.2)" }} />
      <div className="absolute bottom-1/4 right-1/3 w-60 h-60 rounded-full blur-[70px] pointer-events-none z-1"
        style={{ background: "rgba(204,213,174,0.25)" }} />
      ...
      >
        <div className="glass-card p-8 md:p-10">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold mt-4" style={{ color: "#3d3522" }}>Welcome back</h2>
            <p className="text-sm mt-1" style={{ color: "#a08c6a" }}>Sign in to your recruiter account</p>
          </div>
          ...
          <p className="text-center text-sm mt-6" style={{ color: "#a08c6a" }}>
            New to HireSense?{" "}
            <Link to="/register" className="font-semibold transition" style={{ color: "#d4a373" }}>Create account</Link>
          </p>
        </div>
      </motion.div>
    </div>
